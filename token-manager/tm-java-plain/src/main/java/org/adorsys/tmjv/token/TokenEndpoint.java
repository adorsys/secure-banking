...
public class TokenEndpoint {
	...
	public Response token(@Context HttpServletRequest request, Jwt jwt){
		...
		if(StringUtils.isNotBlank(jwt.getAud()))builder.audience(jwt.getAud());
		if(jwt.getExp()!=null)builder.expirationTime(jwt.getExp());
		if(StringUtils.isNotBlank(jwt.getIss()))builder.issuer(jwt.getIss());
		if(jwt.getIat()!=null)builder.issueTime(jwt.getIat());
		if(StringUtils.isNotBlank(jwt.getJti()))builder.jwtID(jwt.getJti());
		if(jwt.getNbf()!=null)builder.notBeforeTime(jwt.getNbf());
		if(StringUtils.isNotBlank(jwt.getSub()))builder.subject(jwt.getSub());
		if(StringUtils.isNotBlank(jwt.getAud()))builder.claim("cnf", jwt.getCnf());
